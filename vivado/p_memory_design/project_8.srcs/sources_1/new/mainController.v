...
module mainController (
    ...
    always @ (state, xStart, mxnControllerEnd , weightPointerControllerEnd, previousYControllerEnd,yPointerControllerEnd,endNeuron,endLayer) // Form the next state
        case (state)
            S0: next_state = S1;
            S1: if(xStart)next_state = S2; else next_state = S1;
            S2: next_state = S3;
            S3: next_state = S4;
            S4: next_state = S5;
            ...
            S6: next_state = S7;
            S7: next_state = S8;   
            S8: if(!weightPointerControllerEnd || !previousYControllerEnd) next_state = S8;
                else if (weightPointerControllerEnd && previousYControllerEnd) next_state = S9;
            S9: next_state = S3;
            S10: next_state = S11;
            S11: next_state = S12;
    ...
    always @ (state, xStart, mxnControllerEnd , weightPointerControllerEnd, previousYControllerEnd,yPointerControllerEnd,endNeuron,endLayer) // Form the Mealy output
        case (state)
            S0: begin 
                xEnd = 0;
                weightPointerControllerStart = 0;
                previousYControllerStart = 0;
